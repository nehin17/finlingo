...
export default function MiniAnalystChallenge() {
  ...
  const expertAnalysis = 'Company A demonstrates stronger long-term economics. While Company B has shown respectable growth, Company A superior margins (62% gross, 22% FCF) indicate better pricing power and operational efficiency.'
      ...
      >
        ...
        <div className="mb-8 overflow-x-auto rounded-lg border border-border">
          <table className="w-full">
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} className="border-t border-border hover:bg-surface-elevated/20 transition-colors">
                  <td className="py-4 px-6 text-text-primary font-semibold">{row.metric}</td>
                  <td className="text-center py-4 px-6 text-lg font-bold text-primary">{row.companyA}</td>
                  <td className="text-center py-4 px-6 text-lg font-bold text-text-secondary">{row.companyB}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mb-8">
          <label className="text-base font-semibold text-text-primary block mb-3">Your Analysis</label>
          <textarea
            value={userAnswer}
            onChange={(e) => setUserAnswer(e.target.value)}
            placeholder="Type your 2–3 sentence analysis here..."
            className="w-full h-24 p-4 rounded-lg border border-border outline-none focus:border-primary transition-colors resize-none"
            style={{
        ...
        <div className="flex gap-3 flex-wrap mb-8">
          <button
            ...
            className="px-6 py-3 rounded-lg bg-primary text-white font-semibold hover:bg-primary/90 disabled:bg-primary/50 disabled:cursor-not-allowed transition-all text-base"
          >
            Compare My Answer
          </button>
          <button
            onClick={() => setShowAnswer(!showAnswer)}
            className="px-6 py-3 rounded-lg border border-primary text-primary font-semibold hover:bg-primary/10 transition-all text-base flex items-center gap-2"
          >
            {showAnswer ? 'Hide' : 'Reveal'} Analyst Explanation
            <ChevronDown size={18} className={`transition-transform ${showAnswer ? 'rotate-180' : ''}`} />
          </button>
        </div>
      ...
      {showComparison && (
        ...
        >
          <motion.div
            ...
            className="rounded-2xl p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-border"
            style={{ background: 'var(--surface)' }}
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <div className="rounded-lg border border-border p-6" style={{ background: 'var(--surface-elevated)' }}>
              ...
              <div className="rounded-lg border border-primary/30 p-6" style={{ background: 'var(--surface-elevated)' }}>
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center text-sm font-bold text-primary">
                    ✓
                  </div>
            ...
            <div className="p-6 rounded-lg border-l-4" style={{ background: 'var(--surface-elevated)', borderColor: 'var(--primary)' }}>
            ...
            <div className="flex gap-3 mt-8 flex-wrap">
              <button
                onClick={() => setShowComparison(false)}
                className="flex-1 px-6 py-3 rounded-lg bg-primary text-white font-semibold hover:bg-primary/90 transition-all"
              >
                Close
              </button>
              <button
                onClick={handleTryAgain}
                className="flex-1 px-6 py-3 rounded-lg border border-primary text-primary font-semibold hover:bg-primary/10 transition-all"
              >
                Try Again
              </button>
            </div>
